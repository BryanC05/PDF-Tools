...
import { Upload, Loader2, Download, X, Archive } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import axios from 'axios';
import { API_URL } from '../lib/config';

interface PdfToPdfaModalProps {
    isOpen: boolean;
    onClose: () => void;
}

export function PdfToPdfaModal({ isOpen, onClose }: PdfToPdfaModalProps) {
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/pdf-to-pdfa`, formData);
            setResultUrl(`${API_URL}${response.data.url}`);
        } catch (error: any) {
            alert(error?.response?.data?.detail || 'Conversion to PDF/A failed.');
        } finally {
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-gray-100">
                            <h3 className="font-semibold text-gray-800 flex items-center gap-2">
                                <Archive className="w-5 h-5 text-amber-600" /> PDF to PDF/A
                            </h3>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <p className="text-xs text-gray-400 text-center">Converts PDF to PDF/A-2B format for long-term archival and compliance</p>
                            {resultUrl && (
                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-amber-50 border border-amber-200 rounded-xl space-y-3">
                                    <div className="flex items-center gap-2 text-amber-700 font-medium text-sm">
                                        <Archive className="w-4 h-4" /> Converted to PDF/A-2B
                                    </div>
                                    <a href={resultUrl} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors font-medium">
                        ...
                        <div className="p-4 border-t border-gray-100 flex justify-end gap-3">
                            ...
                            <button onClick={handleProcess} disabled={!pdfFile || isProcessing}
                                ...
                                {isProcessing ? <><Loader2 className="w-4 h-4 animate-spin" /> Converting...</> : 'Convert to PDF/A'}
                            </button>
                        </div>
                    </motion.div>
                </div>
