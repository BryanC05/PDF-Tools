...
export function PdfToWordModal({ isOpen, onClose }: PdfToWordModalProps) {
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-gray-100">
                            <h3 className="font-semibold text-gray-800 flex items-center gap-2">
                                <FileText className="w-5 h-5 text-blue-600" /> PDF to WORD
                            </h3>
                        ...
                        <div className="p-4 border-t border-gray-100 flex justify-end gap-3">
                            ...
                            <button onClick={handleProcess} disabled={!pdfFile || isProcessing}
                                ...
                                {isProcessing ? <><Loader2 className="w-4 h-4 animate-spin" /> Converting...</> : 'Convert to DOCX'}
                            </button>
                        </div>
                    </motion.div>
                </div>
