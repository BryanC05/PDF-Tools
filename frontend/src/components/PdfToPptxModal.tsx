...
interface PdfToPptxModalProps {
    isOpen: boolean;
    onClose: () => void;
}

export function PdfToPptxModal({ isOpen, onClose }: PdfToPptxModalProps) {
    const [pdfFile, setPdfFile] = useState<File | null>(null);
    const [isProcessing, setIsProcessing] = useState(false);
    const [result, setResult] = useState<{ url: string; slides: number } | null>(null);
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/pdf-to-pptx`, formData);
            setResult({ url: `${API_URL}${response.data.url}`, slides: response.data.slides });
        } catch (error: any) {
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-gray-100">
                            <h3 className="font-semibold text-gray-800 flex items-center gap-2">
                                <Presentation className="w-5 h-5 text-orange-500" /> PDF to POWERPOINT
                            </h3>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <p className="text-xs text-gray-400 text-center">Each PDF page will be converted to a PowerPoint slide as an image</p>
                            {result && (
                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-orange-50 border border-orange-200 rounded-xl space-y-3">
                                    <p className="text-sm text-gray-600">Created <span className="font-medium text-orange-600">{result.slides}</span> slides</p>
                                    <a href={result.url} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors font-medium">
                        ...
                        <div className="p-4 border-t border-gray-100 flex justify-end gap-3">
                            ...
                            <button onClick={handleProcess} disabled={!pdfFile || isProcessing}
                                ...
                                {isProcessing ? <><Loader2 className="w-4 h-4 animate-spin" /> Converting...</> : 'Convert to PPTX'}
                            </button>
                        </div>
                    </motion.div>
                </div>
