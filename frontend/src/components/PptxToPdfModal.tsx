...
export function PptxToPdfModal({ isOpen, onClose }: PptxToPdfModalProps) {
    ...
    const onDrop = useCallback((acceptedFiles: File[]) => {
        const pdf = acceptedFiles.find(f => f.type === 'application/pdf' || f.type === 'application/vnd.openxmlformats-officedocument.presentationml.presentation');
        if (pdf) {
            setPdfFile(pdf);
        }
    }, []);
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    ...
                    >
                        ...
                        <div className="p-6 space-y-4">
                            <div
                                {...getRootProps()}
                                className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all ${isDragActive ? 'border-orange-400 bg-orange-50' : 'border-gray-200 hover:border-orange-300 hover:bg-orange-50/50'}`}
                            >
                                <input {...getInputProps()} />
                                ...
                                {pdfFile ? (
                                    ...
                                    <p className="text-gray-500 text-sm">
                                        {isDragActive ? 'Drop file here...' : 'Drag & drop a PowerPoint file'}
                                    </p>
                                )}
                            </div>

                            <BackendRequiredBanner
                                featureName="POWERPOINT to PDF Conversion"
                                description="Converting PowerPoint presentations to PDF requires LibreOffice or similar server-side processing."
                            />
                        </div>
