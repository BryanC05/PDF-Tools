...
import { extractPages } from '../lib/pdfUtilsClient';
...
export function ExtractPagesModal({ isOpen, onClose }: ExtractPagesModalProps) {
    const [pdfFile, setPdfFile] = useState<File | null>(null);
    const [pages, setPages] = useState('');
    const [isProcessing, setIsProcessing] = useState(false);
    const [result, setResult] = useState<{ url: string; extracted_pages: number } | null>(null);
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-gray-100">
                            <h3 className="font-semibold text-gray-800 flex items-center gap-2">
                                <FileOutput className="w-5 h-5 text-indigo-500" /> Extract Pages
                            </h3>
                        ...
                        <div className="p-6 space-y-4">
                            <div {...getRootProps()} className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${isDragActive ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 hover:border-indigo-300 hover:bg-indigo-50/50'}`}>
                                <input {...getInputProps()} />
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Pages to extract (e.g., 1,3,5-7)</label>
                        ...
                        <div className="p-4 border-t border-gray-100 flex justify-end gap-3">
                            ...
                            <button onClick={handleProcess} disabled={!pdfFile || !pages.trim() || isProcessing}
                                className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium transition-all shadow-lg shadow-indigo-500/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
                                {isProcessing ? <><Loader2 className="w-4 h-4 animate-spin" /> Extracting...</> : 'Extract Pages'}
                            </button>
                        </div>
                    </motion.div>
                </div>
